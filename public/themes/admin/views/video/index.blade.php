        ...
        <div class="layui-col-md12">
            <div class="tabel-message">
                <div class="layui-inline tabel-btn">
                    <button class="layui-btn layui-btn-warm "><a href="{{guard_url('video/create')}}">{{ trans('app.add') }}</a></button>
...
<script>
    var main_url = "{{guard_url('video')}}";
    var delete_all_url = "{{guard_url('video/destroyAll')}}";
    layui.use(['jquery','element','table'], function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                ,{field:'title',title:'{{ trans('app.title') }}',edit:'text' ,width:300}
                ,{field:'vid',title:'{{ trans('video.label.vid') }}',edit:'text', width:300}
                ,{field:'image',title:'{{ trans('video.label.image') }}', toolbar:'#imageTEM', width:120}
        ...
        form.on('switch(home_recommend)', function(obj){
            ...
            var load = layer.load();
            $.post("{{guard_url('video/updateRecommend')}}",{"id":this.value,"home_recommend":value,'_token':"{!! csrf_token() !!}" },function(){
                layer.close(load);
                if(data.code != 0)
                {
        ...
        form.on('switch(hot_recommend)', function(obj){
            var  value = obj.elem.checked ? 1 : 0;
            var load = layer.load();
            $.post("{{guard_url('video/updateRecommend')}}",{"id":this.value,"hot_recommend":value,'_token':"{!! csrf_token() !!}" },function(){
                layer.close(load);
                if(data.code != 0)
                {
                    layer.msg(data.message);
